...
class RegBN(nn.Module):
    ...
    def __init__(
        ...
        assert beta1 < 1, ValueError("Invalid beta1: {}".format(beta1))

        assert beta2 < 1, ValueError("Invalid beta2: {}".format(beta2))
        ...
        self.norm_f = _get_norm_inp(
            normalize_input, f_num_channels, f_layer_dim, affine
        )
        self.norm_g = _get_norm_inp(
            normalize_input, g_num_channels, g_layer_dim, affine
        )

        self.norm_f_out = _get_norm_out(
            normalize_output, f_num_channels, f_layer_dim, affine
        )
        self.norm_g_out = _get_norm_out(
            normalize_output, g_num_channels, g_layer_dim, affine
        )

    def _reset_adam_prams(
        self,
        momentum: float = 0.1,
    ) -> None:
    ...
    def update_W(
        ...
        with torch.no_grad():
            ...
            self.W = (1.0 - eta) * self.W + eta * W_cur
    ...
    def forward(
        ...
        if is_training:
            # svd decomposition
            g_u, g_s_diag, g_v = _svd_decomposition(
                g_flat_nor, self.sigma_THR, self.sigma_MIN
            )
            if g_u is None:
                self.is_nan_ = True
                return f, g
            ...
            # estimate W
            W_plus, lambda_, not_found = self.W_calc.compute(
                g_flat_nor, g_u, g_s_diag, g_v, self.lambda_set
            )
    ...
    def extra_repr(self) -> str:
        return (
            "f_num_channels={f_num_channels}, g_num_channels={g_num_channels}".format(
                **self.__dict__
            )
        )
...
class proj_matrix_estimator(object):
    ...
    def lambda_fn(
        self, x: Tensor, lambda_: Tensor, u: Tensor, s_diag: Tensor
    ) -> torch.Tensor:
    ...
    def get_W_plus(self, lambda_, u, s_diag, v) -> Tensor:
        ...
        sl_inv = torch.nan_to_num(
            sl_inv, nan=epsilon_, posinf=epsilon_, neginf=epsilon_
        )
        S = torch.mm(sl_inv, torch.diag(s_diag))

        W = torch.mm(torch.mm(torch.conj(u), S), v.t()).t()
        return W
    ...
    def compute(
        ...
        if len(lambda_set) > 3:
            lambda_init = [
                coef * torch.median(lambda_set).item()
                for coef in [0.001, 0.01, 0.1, 1.0, 10.0]
            ]
            ...
            for lambd_ini in lambda_init:
                ...
                if abs(loss_lbfgs[-1]) > self.pred_tolerance:
                    ...
                    lbfgs_optim = torch.optim.LBFGS(
                        params=[lambda_], lr=lr_i, **self.lbfgs_kwargs
                    )

                    lbfgs_optim.step(lbfgs_closure)
                    est_err = self.lambda_fn(
                        x_mat,
                        lambda_,
                        u_mat,
                        s_diag_mat,
        ...
        if (not _lambda_not_found) and (min_loss < 10.0 * self.pred_tolerance):
...
def _svd_decomposition(data, sigma_THR, sigma_MIN):
    ...
    try:
        ...
        return u, s_diag, v
    except Exception:  # Reemplazar except genérico con Exception específico
        try:
            ...
            v = vh.mH if torchV >= 13 else vh  # torch.adjoint(vh)
            return u, s_diag, v
        except Exception:  # Especificar la excepción en lugar de usar except genérico
            return None, None, None


def _get_norm_inp(
    normalize_input: bool, num_channels: int, layer_dim: List, affine: bool
):
